...
Route::group(['prefix' => (LOGIN_USER_TYPE=='company')?'company':'admin', 'middleware' =>'admin_guest'], function () {
	...
	Route::group(['middleware' => 'admin_can:manage_admin'], function() {
		Route::get('barcodes/create', 'QrcodeController@create')->name('qrcode.create');
		Route::match(['GET', 'POST'], 'barcodes/store', 'QrcodeController@store')->name('barcode.store');
		Route::get('barcodes', 'QrcodeController@index')->name('barcode.index');
		Route::match(['GET', 'POST'], 'barcodes/edit/{barcode}', 'QrcodeController@edit')->where('barcode', '[0-9]+')->name('barcode.edit');
		Route::delete('barcodes/destroy/{barcode}', 'QrcodeController@destroy')->where('barcode', '[0-9]+')->name('barcode.destroy');
		Route::put('barcodes/update/{barcode}', 'QrcodeController@update')->where('barcode', '[0-9]+')->name('barcode.update');
		Route::get('barcodes/{barcode}/toggle-status', 'QrcodeController@toggleStatus')->where('barcode', '[0-9]+')->name('barcode.toggleStatus');
		Route::get('barcodes/download/{card_number}', 'QrcodeController@download')->name('barcode.download');
	});
